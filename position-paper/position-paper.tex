...
\begin{abstract}
    ...
    \emph{Sequence Conflict-free Replicated Data Types} are used to replicate and edit, without any kind of coordination, sequences in distributed systems.
    ...
    In this paper, we present our ongoing work on the design and validation of a fully distributed renaming mechanism, setting a bound to the footprint.
...
The second approach \cite{AndreCollaborateCom2013} proposes to attach an identifier to each inserted element.
...
Nonetheless, using consensuses is expensive and is not suited to all kinds of applications. In a fully distributed application, there is no central authority able to provide a set of stable nodes acting as the \emph{Core}. A fully distributed and efficient renaming mechanism is thus needed.
...
We intend to measure common metrics like the memory and CPU usage, bandwidth consumption but also more specific metrics like the time to propagate and integrate an update and the global time required to converge.
